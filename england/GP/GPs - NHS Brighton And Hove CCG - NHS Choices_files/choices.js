...
var alertBannerStr = "<div class='banner'>" +
    "[Content]" +
    "<p class='updated'>Last update:<br />[FormattedCreatedDate]</p>" +
    "<span id='closealert'>Close &nbsp;&nbsp;&nbsp;X</span>" +
    "</div>";
...
$(document).ready(function () {
    ...
    var url = isTestMode() ? 
        ...
        :"/externalservices/surveyfeedapi/api/bannerfeed";
...
function fRemoveKeypressListenerFromTextAreas() {
    aTextAreas = document.getElementsByTagName('textarea');
    nTextAreaLength = aTextAreas.length;
    for (nTextAreaCount = 0; nTextAreaCount < nTextAreaLength; nTextAreaCount++) {
        aTextAreas[nTextAreaCount].addEventListener('keypress', TextareaKeypressHandler, false);
    }
}
...
// ----------------------
// FUNCTION: Leaving site warning
// DESCRIPTION: displays a warning for SOME external links
// ----------------------
$(document).ready(function () {
    ...
    var targetUrl;
    $("#dialog-confirm").append(dialogText);
    ...
    $('a.ext-confirm').on('click', function () {
...
// -----------------------------------------------------------------------------
/* F&C Infobox link fix */
// -----------------------------------------------------------------------------
jQuery(document).ready(function ($) {
    if ($("#myMap").length) {
        $(document).on('touchstart', '#myMap .maptitle a', function (e) { window.location.href = $(this).attr('href'); });
    }
});
// -----------------------------------------------------------------------------
...
$(document).ready(function () {
 ...
 $seenBannerCookie = $.cookie(cookieName);
    ...
    var closeBanner = function () {
        // Firefox version 48 and lower doesn't support 'background-position-y' so animating it won't work
        if ($.browser.mozilla && $.browser.version < '49.0') {
            var backgroundPos = $('.wrap').css('backgroundPosition').split(" ")
            var backgroundPosY = backgroundPos[1].split("px");
            $('body').css('background-position', 'center ' + (parseInt(backgroundPosY[0]) + 5) + 'px');
            $('#' + bannerId).hide();
        }
        else {
